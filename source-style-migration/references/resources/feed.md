...
- image gen target aspect: `503:645` (`~0.78:1`).
- Renderer input filename: `feed.png`
- Layout guide: `references/layout-guides/feed-layout-guide.png`
- image gen owns title/subtitle text rendering. Renderer owns CTA drawing, dimensions, and final technical validation.

## Generation Layout

- Use `feed-layout-guide.png` as the generation-stage structure reference.
- In the generation prompt, state the canvas/aspect as `503:645` (`~0.78:1`).
...
- The candidate follows `feed-layout-guide.png`, or has an acceptable semantic layout deviation under `references/shared/visual-qa.md`.
...
- The core subject group is horizontally centered inside the main visual block, with its visual center aligned to the block's horizontal centerline and balanced left/right breathing room.
  核心主体组在主视觉区内水平居中，视觉中心对齐主视觉区水平中线，左右留白相对均衡。
- The title/subtitle group top falls within the target `10% ±1%` of canvas height after proportional scaling; the group remains horizontally centered, readable, unclipped, and separated from the real main visual. Its height is not checked against the blue guide shape.
...
- The title-group top is outside the target `10% ±1%` of canvas height after proportional scaling.
...
After Pre-render QA passes, including `PASS with observation`, or after the allowed one-retry title-Y warning, save the template-safe background as `feed.png` in the task input folder.
