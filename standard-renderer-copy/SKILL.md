...
2. If the image has no discernible main title or subtitle, first classify it before creating a task folder:
   ...
   - For direct horizontal routes only, run `python3 scripts/match-resource-size.py --image <image-path>`. `1041:225` (about `4.63:1`) routes to `channel`; exact `1041:217` routes to `categoryBanner`; and `1035:390` (within 1% ratio tolerance) routes to `push`. All three direct routes use the uploaded no-title image as a renderer-ready background and skip AI generation; Push does not require a confirmation for a ratio match. Any matched image that contains either discernible title must follow the title-cleanup generation path, not direct rendering. If the user names a resource outside the matched route, ask which route they intend before proceeding.
...
- A material request defaults to the original-image background, CTA `点击查看`, automatic title/subtitle color, and template fonts. When the reference has no effective environmental background (for example, a transparent cutout, a flat single-color background, or an isolated product on black/white), default the background expression to `极简光影底`: preserve the product and introduce only subtle, low-saturation light, gradient, or ground reflection drawn from the product's own colors. A user-specified background expression or edge-exit requirement always takes precedence. Resolve automatic title/subtitle color with `references/shared/copy-safety.md` before background generation; write the resolved hex colors to `material.json` before renderer.
- In the no-title branch, `标准文字套版` and `底图套版` are distinct: standard text templating generates a `9:16` opening-screen master from ordinary product/source imagery before renderer; background templating uses an already-complete `9:16` opening-screen background directly. Every request containing `feed`, `popup`, or `splash` uses a `9:16` opening-screen master, whether the source contains title/subtitle or not.
...
- When one request contains one or more of `feed`, `popup`, and `splash`, use one shared `9:16` vertical master. Measure the master core-subject vertical center, write it as `verticalMaster.subjectCenterY` in `material.json`, and let renderer derive each requested final crop. Run Pre-render QA separately on every requested final crop; an explicit resource `backgroundPosition` is an exception-only override.
...
- Every new task folder must use `qaSchemaVersion: 4`. For `feed`, `popup`, and `splash` only, `mainVisualCentered` must be supported by the normalized `centeringMeasurement` defined in `references/shared/visual-qa.md`; the sole exception is the documented `poolCueRightBiased` 台球杆 mode, which uses `poolCueMeasurement` and `poolCueRightBiased` instead. `feed` and `popup` additionally require the pixel-based `fitMeasurement` and `mainVisualFits` gate in either mode. When the reference image has deliberate edge exits, declare `referenceEdgeCrops` and include the required per-resource preservation check and measurement. A user may explicitly narrow the contract to selected exit edges; otherwise it preserves the reference's exact exit edges, direction, continuity, and relative occlusion. After any `feed` or `popup` targeted retry, include the required `retryPlan` audit record so the measured scale, directions, edge-contract clause, and exact image-generation prompt are validated together. `channel`, `categoryBanner`, and `push` do not use these centering/fit gates.
...
10. On the AI-generation path, if Pre-render QA fails:
    ...
    - for `channel` and `categoryBanner`, a split main visual in the first candidate requires one targeted regeneration. Replace the white buffer with continuous scene background; visually measure the complete main visual as normalized `left,top,right,bottom`, then call `scripts/prepare-channel-input.py --main-visual-bounds <left,top,right,bottom>` so the final crop is vertically centered on the measured group. If the retry still has a crop or balance issue, write `RENDER_WITH_OBSERVATION` and continue to renderer with a concrete observation. Other channel balance observations retain the advisory flow. When the candidate is shared, the targeted regeneration edits that one shared candidate and replaces both derived inputs.
11. After Pre-render QA passes, after a permitted `RENDER_WITH_OBSERVATION`, or after an advisory `channel` QA observation, read `references/shared/renderer-flow.md`:
    ...
    - for generated `channel` or `categoryBanner`, run `scripts/prepare-channel-input.py` with the matching `--template` before mapping its image in `material.json`; for a continuous-background recovery candidate, also pass its QA-measured `--main-visual-bounds <left,top,right,bottom>`. When both are requested, run it twice against the same shared horizontal work image, once for each template. Direct no-main-copy images that route to either supported horizontal resource bypass it
...
```text
image inspection
-> has title/subtitle: extract copy -> clear only original main title/subtitle -> requested vertical resource: generate 9:16 opening-screen master -> derive requested vertical renderer output
                                                               -> requested horizontal resource: generate matching resource background -> renderer
