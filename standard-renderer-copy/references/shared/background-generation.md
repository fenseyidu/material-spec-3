...
For a request that contains one or more of `feed`, `popup`, and `splash`, generate one shared `9:16` vertical master using the splash composition as the base. Keep a naturally blended, continuous title/subtitle support area across the full upper area needed by every derived final crop; keep the complete core subject group in the common usable region, with clear space below it for the stricter feed/popup button-lower-edge bands. After generation, measure the complete core-subject vertical center in the master and use it to derive each requested renderer crop.
...
When that first-generation candidate splits a core subject at the white-buffer boundary, its one targeted regeneration switches to continuous-background recovery. The single correction prompt must require the white buffer to become a natural continuation of the existing scene background, while moving or scaling the entire measured main visual toward the final horizontal crop. Do not request a new white boundary on this retry. Pre-render QA records the complete main-visual bounds, and `prepare-channel-input.py --main-visual-bounds` uses their vertical center to derive the crop. If a remaining edge crop is still visible after that retry, record it as `RENDER_WITH_OBSERVATION` and continue to renderer.
